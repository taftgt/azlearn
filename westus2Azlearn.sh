...
az vm create --resource-group $rg --image Canonical:0001-com-ubuntu-server-focal:20_04-lts:latest --admin-username radium --admin-password Ferozataftgt88 --no-wait --custom-data downloader.sh --size "Standard_DS2_v2" --location westus2 --name SampleVM1

az vm create --resource-group $rg --image Canonical:0001-com-ubuntu-server-focal:20_04-lts:latest --admin-username radium --admin-password Ferozataftgt88 --no-wait --custom-data downloader.sh --size "Standard_DS2_v2" --location westus2 --name SampleVM2
